...
var b = { regex: { rules: { class: { pattern: /^\.([a-zA-Z][\w-]*)$/, resolve(e) {
  ...
  if (s2(t) && r.transform !== "strict") {
    let y = e.replace("-directive", ""), m2 = t, { colorScheme: R2, extend: T, css: j } = m2, O = v2(m2, ["colorScheme", "extend", "css"]), d = T || {}, { colorScheme: M } = d, z3 = v2(d, ["colorScheme"]), u = R2 || {}, { dark: V } = u, L = v2(u, ["dark"]), g2 = M || {}, { dark: te } = g2, re2 = v2(g2, ["dark"]), K2 = s2(O) ? this._toVariables({ [y]: h(h({}, O), z3) }, r) : {}, A2 = s2(L) ? this._toVariables({ [y]: h(h({}, L), re2) }, r) : {}, X = s2(V) ? this._toVariables({ [y]: h(h({}, V), te) }, r) : {}, [G2, U2] = [(f2 = K2.declarations) != null ? f2 : "", K2.tokens || []], [B, I] = [(x2 = A2.declarations) != null ? x2 : "", A2.tokens || []], [H2, W2] = [(p = X.declarations) != null ? p : "", X.tokens || []], q2 = this.transformCSS(y, `${G2}${B}`, "light", "variable", r, i2, a2, n), Z = this.transformCSS(y, H2, "dark", "variable", r, i2, a2, n);
    l2 = `${q2}${Z}`, o = [.../* @__PURE__ */ new Set([...U2, ...I, ...W2])], c2 = m(j, { dt: E });
  }
  ...
  let a2 = function(l2, o = {}, c2 = []) {
    ...
    Object.entries(l2).forEach(([m2, d]) => {
      ...
      i(d) ? n(d, u, g2) : (i2[u] || (i2[u] = { paths: [], computed: (f2, x2 = {}, p = []) => {
...
var BaseStyle = class _BaseStyle {
  ...
  classes = {};
  inlineStyles = {};
  load = (style7, options = {}, transform = (cs) => cs) => {
    const computedStyle = transform(ar`${m(style7, {
      dt: E
    })}`);
  ...
  loadStyle = (options = {}, style7 = "") => {
    return this.load(this.style, options, (computedStyle = "") => S.transformCSS(options.name || this.name, `${computedStyle}${ar`${style7}`}`));
  };
  loadBaseCSS = (options = {}) => {
...
var DomHandler = class _DomHandler {
  ...
  static matches(element, selector) {
    ...
    var f2 = p["matches"] || p.webkitMatchesSelector || p["mozMatchesSelector"] || p["msMatchesSelector"] || function(s4) {
  ...
  static getOuterWidth(el, margin) {
    ...
    if (margin) {
      let style7 = getComputedStyle(el);
      width += parseFloat(style7.marginLeft) + parseFloat(style7.marginRight);
    }
    return width;
  }
  static getHorizontalPadding(el) {
    let style7 = getComputedStyle(el);
    return parseFloat(style7.paddingLeft) + parseFloat(style7.paddingRight);
  }
  static getHorizontalMargin(el) {
    let style7 = getComputedStyle(el);
    return parseFloat(style7.marginLeft) + parseFloat(style7.marginRight);
  }
  static innerWidth(el) {
    let width = el.offsetWidth;
    let style7 = getComputedStyle(el);
    width += parseFloat(style7.paddingLeft) + parseFloat(style7.paddingRight);
    return width;
  }
  static width(el) {
    let width = el.offsetWidth;
  ...
  static getInnerHeight(el) {
    ...
    let style7 = getComputedStyle(el);
    height += parseFloat(style7.paddingTop) + parseFloat(style7.paddingBottom);
    return height;
  }
  static getOuterHeight(el, margin) {
    ...
    if (margin) {
      let style7 = getComputedStyle(el);
      height += parseFloat(style7.marginTop) + parseFloat(style7.marginBottom);
    }
    return height;
  }
  static getHeight(el) {
    let height = el.offsetHeight;
    let style7 = getComputedStyle(el);
    height -= parseFloat(style7.paddingTop) + parseFloat(style7.paddingBottom) + parseFloat(style7.borderTopWidth) + parseFloat(style7.borderBottomWidth);
    return height;
  }
  static getWidth(el) {
    let width = el.offsetWidth;
    let style7 = getComputedStyle(el);
    width -= parseFloat(style7.paddingLeft) + parseFloat(style7.paddingRight) + parseFloat(style7.borderLeftWidth) + parseFloat(style7.borderRightWidth);
    return width;
  }
  static getViewport() {
    let win = window, d = document, e = d.documentElement, g2 = d.getElementsByTagName("body")[0], w3 = win.innerWidth || e.clientWidth || g2.clientWidth, h2 = win.innerHeight || e.clientHeight || g2.clientHeight;
    return { width: w3, height: h2 };
  }
  static getOffset(el) {
  ...
  static calculateScrollbarWidth(el) {
    if (el) {
      let style7 = getComputedStyle(el);
      return el.offsetWidth - el.clientWidth - parseFloat(style7.borderLeftWidth) - parseFloat(style7.borderRightWidth);
    } else {
...
var _c027 = ["data-p-icon", "caret-left"];
...
var _c028 = ["data-p-icon", "caret-right"];
...
var _c030 = ["data-p-icon", "exclamation-triangle"];
...
var _c032 = ["data-p-icon", "eyeslash"];
...
var _c034 = ["data-p-icon", "filter-slash"];
...
var _c036 = ["data-p-icon", "info-circle"];
...
//# sourceMappingURL=chunk-KM4OENLW.js.map

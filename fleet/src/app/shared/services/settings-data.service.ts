...
export class SettingsDataService {
  ...
  getVessels(forceRefresh = false): Observable<VesselSettingsRecord[]> {
    ...
    if (!this.backendVessels$) {
      ...
      this.backendVessels$ = (
        cachedRows.length ? concat(of(cachedRows), backendRows$) : backendRows$
  ...
  private loadDatabaseUserAccess(
    ...
    return request$.pipe(
      timeout(endpoint.timeoutMs),
      map((response: any) =>
        this.extractArray(response).map((row, index) =>
          this.normalizeUserAccess(row, index, 'backend'),
        ),
      ),
    );
  }
  ...
  private getCachedOverviewVessels(): VesselSettingsRecord[] {
    ...
    return snapshot
      .map((row: any, index: number) => this.normalizeVessel(row?.fvInfo ?? row?.fv ?? row, index))
  ...
  private normalizeVessel(row: any, index: number): VesselSettingsRecord {
    ...
    const status: VesselSettingsStatus =
      derivedStatus === 'nodata' ? explicitStatus : derivedStatus;
    ...
    if (Array.isArray(value)) {
      const normalized = value
        .map((item: any, index: number): VesselEngineAssignment | null => {
          ...
          const sourceValue = String(item?.source ?? item?.Source ?? '')
            .trim()
            .toLowerCase();
          ...
          return {
            ...
            position: String(item?.position ?? item?.Position ?? `Engine ${index + 1}`).trim(),
            ...
            realtimeType:
              String(item?.realtimeType ?? item?.RealtimeType ?? '').trim() || undefined,
            source:
              sourceValue === 'realtime' || sourceValue === 'manual' || sourceValue === 'legacy'
  ...
  private normalizeUserAccess(
    ...
    const groupIds = this.toStringArray(
      row?.groupIds ?? row?.GroupIds ?? row?.groups ?? row?.Groups,
    );
    ...
    const scopeText = String(
      row?.accessScope ?? row?.AccessScope ?? row?.scope ?? '',
    ).toLowerCase();
    const accessScope =
      scopeText === 'groups' || scopeText === 'vessels' || scopeText === 'all'
        ? scopeText
        : role === 'administrator'
          ? 'all'
          : groupIds.length
    ...
    return {
      ...
      role,
      status:
        String(row?.status ?? row?.Status ?? 'active').toLowerCase() === 'suspended'
          ? 'suspended'
          : 'active',
      accountId:
        String(
          row?.accountId ?? row?.AccountId ?? row?.identityId ?? row?.IdentityId ?? '',
        ).trim() || undefined,
      accountProvisioning:
        row?.accountProvisioning === 'managed' || row?.accountProvisioning === 'linked'
      ...
      groupIds,
      vesselIds,
      additionalVesselIds: this.toStringArray(row?.additionalVesselIds ?? row?.AdditionalVesselIds),
  ...
  private normalizeModulePermissions(value: any, role: UserAccessRole): UserModulePermissionMap {
    ...
    return modules.reduce((permissions, module) => {
      const raw =
        value?.[module] ?? value?.[module.replace('-', '')] ?? value?.[module.toUpperCase()];
      const roleView = role === 'administrator' || module !== 'settings';
      const view = selected
        ? selected.has(module) ||
          (module === 'data-logger' && selected.has('datalogger')) ||
          (module === 'alarm' && selected.has('alerts'))
        : Boolean(raw?.view ?? raw?.View ?? roleView);
      permissions[module] = {
        view,
        export: Boolean(
          raw?.export ?? raw?.Export ?? (view && module !== 'overview' && module !== 'settings'),
        ),
  ...
  private currentSessionRecord(): UserSessionRecord {
    ...
    const username =
      ...
      'Current user';
    const role = String(
      rawUser?.role ?? rawUser?.Role ?? rawUser?.Group ?? rawUser?.group ?? 'Administrator',
    );
    const now = new Date().toISOString();
    ...
    if (Array.isArray(value)) {
      return value
        .map((item: any) => {
          ...
          return String(
            item?.id ?? item?.Id ?? item?.vesselId ?? item?.name ?? item?.Name ?? '',
          ).trim();
        })
        .filter(Boolean);
    }

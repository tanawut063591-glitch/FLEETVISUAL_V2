...
export class DataLoggerComponent implements OnInit, OnDestroy {
    ...
    private rebuildPagination() {
        this.totalRows = Math.floor((this.end.getTime() - this.start.getTime()) / this.selectedIntervalMs) + 1;
            ...
            this.requestSub = this.newHttp.getHistorianValues(startTime, endTime, tags).subscribe(
                (res: any) => {
                    ...
                    if (rawData.length === 0 || dictionary.size === 0) {
                        this.errorMessage = 'No historian data returned for the selected vessel, tags and time range.';
                    }

                    const stArr = this.parseDateValue(startTime).getTime();
                    const enArr = this.parseDateValue(endTime).getTime();

                    const reportValue = this.processTimeSeries(
                        stArr,
                        enArr,
                        this.headers,
                        dictionary
        ...
        this.ngZone.runOutsideAngular(() => {
            this.newHttp.getHistorianValues(startStr, endStr, tags).subscribe(
                (res: any) => {
                    ...
                    const headerNames = [
                        'Time',
                        ...headersDef.map((x: DataLoggerHeader) => x.name)
                    ];

                    const rows = reportValue.map((x: DataLoggerRow) => {
                        return [
                            x.timestamp,
                            ...x.values.map((v: any) => this.exportValue(v))
                        ];
                    });
    ...
    private extractResponseArray(response: any): any[] {
        ...
        if (source && !Array.isArray(source) && typeof source === 'object') {
            const candidates = [
                source.data, source.Data, source.result, source.Result,
                source.results, source.Results, source.records, source.Records,
                source.items, source.Items, source.values, source.Values,
                source.tags, source.Tags, source.HistorianValues, source.historianValues,
                source.history, source.History, source.payload, source.Payload
            ];
    ...
    getRangeLabel(): string {
        ...
        return this.formatNumber(from) + ' – ' + this.formatNumber(to) + ' of ' + this.formatNumber(this.totalRows);
    }
    ...
    private getTagUnit(name: string): string {
        ...
        if (lower.indexOf('fuel') !== -1 || lower.indexOf('fin') !== -1 || lower.indexOf('fout') !== -1 || lower.indexOf('rate') !== -1) {
            return '(L/h)';
        }
        ...
        if (lower.indexOf('lat') !== -1 || lower.indexOf('long') !== -1 || lower.indexOf('lng') !== -1) {
            return '(°)';
        }
    ...
    private getTagCategory(name: string): string {
        ...
        if (lower.indexOf('fuel') !== -1 || lower.indexOf('fin') !== -1 || lower.indexOf('fout') !== -1 || lower.indexOf('cons') !== -1) {
            return 'fuel';
        }

        if (lower.indexOf('gps') !== -1 || lower.indexOf('lat') !== -1 || lower.indexOf('long') !== -1 || lower.indexOf('lng') !== -1 || lower.indexOf('speed') !== -1 || lower.indexOf('sog') !== -1 || lower.indexOf('cog') !== -1 || lower.indexOf('distance') !== -1) {
            return 'navigation';
        }

        if (lower.indexOf('gen') !== -1 || lower.indexOf('generator') !== -1 || lower.indexOf('dg') !== -1) {
            return 'generator';
        }

        if (lower.indexOf('volt') !== -1 || lower.indexOf('battery') !== -1 || lower.indexOf('amp') !== -1) {
            return 'electrical';
        }
        ...
        if (lower.indexOf('env') !== -1 || lower.indexOf('air') !== -1 || lower.indexOf('humidity') !== -1) {
            return 'environment';
        }

...
export class DatetimeControlComponent implements OnInit, OnDestroy, OnChanges {
  ...
  @Input() pageSubtitle =
    'View, export and analyze historical sensor values from the selected vessel.';
  ...
  ngOnInit() {
    ...
    this.activeVesselSub = this.fvRealtimeService.activeVessel$.subscribe((vessel: any) =>
      this.applyActiveVessel(vessel),
    );
  ...
  private resolveVesselPrefix(value: any): string {
    var sources = [value, value && value.fvInfo, value && value.fv, value && value.vessel];
  ...
  openDialog() {
    ...
    if (this.prefix && !this.pointsLoading && this.tags.length === 0) {
      ...
      this.pointErrorMessage =
        'Vessel data is still loading. Please wait a moment or select a vessel from the sidebar.';
    }
  ...
  retryPoints(): void {
    ...
    if (!this.prefix) {
      this.hasPointError = true;
      this.pointErrorMessage =
        'No vessel is available yet. Select a vessel from the sidebar and retry.';
      this.markView();
      return;
    }
  ...
  private processTagsResult(rawTags: any[], prefix: string) {
    ...
    for (var i = 0; i < rawTags.length; i++) {
      ...
      normalized.push({
        ...
        description:
          (raw && (raw.Description || raw.description || raw.Display || raw.display)) || '',
  ...
  getTagSelected() {
    var serviceSelected =
      this.tagService && this.tagService.tagSelected ? this.tagService.tagSelected : [];

    for (var key in this.tagGroup) {
      if (this.tagGroup.hasOwnProperty(key)) {
        var filter = serviceSelected.filter((t: any) => {
          return t && t.group && t.group.join(',') === this.tagGroup[key].group.join(',');
        });
